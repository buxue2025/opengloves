...
- 🎯 **单端口访问** - 内置 WebSocket 代理，仅需开放 8443 端口（HTTPS）
...
> 🆕 **v0.02 新特性**：OpenGloves 现在内置 WebSocket 代理，只需映射一个端口！

### 网络配置

**路由器/防火墙设置**:
```
外部端口 8443 → 内部 Mac Mini:8443 (OpenGloves HTTPS + 内置代理)
```

**或者使用标准 HTTPS 端口**:
```
外部端口 443 → 内部 Mac Mini:8443
```
